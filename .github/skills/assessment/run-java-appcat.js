...
function loadConfig(configPath) {
  ...
  const java = root && typeof root === 'object' && !Array.isArray(root) ? root.java : null;
  ...
  return {
    ...
    analysisCoverage: java.analysisCoverage || 'issue-only',
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, (res) => {
      ...
      res.on('data', (chunk) => {
        ...
        const now = Date.now();
        const pct = totalBytes ? Math.floor((received / totalBytes) * 100) : -1;
        // Throttle: log at most every 3s, and on each new 10% step.
        if (now - lastLoggedAt >= 3000 || (pct >= 0 && pct - lastLoggedPct >= 10)) {
          lastLoggedAt = now;
          lastLoggedPct = pct;
          const mb = (received / 1048576).toFixed(1);
          log(pct >= 0
            ? `Downloading AppCAT: ${pct}% (${mb}/${totalMb} MB)`
            : `Downloading AppCAT: ${mb} MB`);
        }
      });
